...
class FileMergerApp:
    ...
    def create_widgets(self, parent):
        ...
        self.extension_vars = {}
        common_extensions = [
            '.py', '.txt', '.md', '.json', '.ini', '.log','.jinja'
            '.java', '.sql', '.html', '.css', '.js', '.xml', '.yml'
        ]
        ...
        self.ignore_dirs_var = tk.StringVar(value="node_modules, dist, build, target, out")
    ...
    def process_files(self, source_dir, extensions, output_file, ignored_dirs_str):
        self.update_log("Iniciando o processo...")
        self.update_log(f"Extensões a serem buscadas: {', '.join(extensions)}")
        ...
        self.update_log(f"Pastas que serão ignoradas: {', '.join(sorted(list(ignored_dirs_set)))}")

        try:
            file_count = 0
            with open(output_file, 'w', encoding='utf-8', errors='ignore') as outfile:
                self.update_log(f"Arquivo de saída criado em: {output_file}")
                ...
                for root, dirs, files in os.walk(source_dir):
                    ...
                    for filename in files:
                        if any(filename.lower().endswith(ext) for ext in extensions):
                            ...
                            self.update_log(f"Processando: {relative_path}")
                            ...
                            try:
                                ...
                                self.update_log(f"!! ERRO ao ler o arquivo {relative_path}: {e}")

            self.update_log(f"\nPROCESSO CONCLUÍDO!")
            self.update_log(f"Total de {file_count} arquivos foram concatenados.")
            messagebox.showinfo("Sucesso",
                                f"Processo concluído!\n{file_count} arquivos foram salvos em:\n{output_file}")

        except Exception as e:
            self.update_log(f"!! ERRO GERAL: {e}")
