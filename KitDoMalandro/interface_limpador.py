...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Radiobutton(
            modo_frame,
            text="Arquivo Único",
            variable=self.modo_operacao_var,
            value="arquivo",
            command=self.toggle_modo_operacao
        ).grid(row=0, column=0, sticky="w")

        ttk.Radiobutton(
            modo_frame,
            text="Pasta (Recursivo)",
            variable=self.modo_operacao_var,
            value="pasta",
            command=self.toggle_modo_operacao
        ...
        self.entrada_entry = ttk.Entry(self.input_frame, textvariable=self.arquivo_entrada_path, state="readonly",
                                       font=("Segoe UI", 9))
        ...
        self.in_place_check = ttk.Checkbutton(
            self.opcoes_frame,
            text="Modificar arquivo original (padrão)",
        ...
        self.saida_entry = ttk.Entry(self.opcoes_frame, textvariable=self.arquivo_saida_path, state="disabled",
                                     font=("Segoe UI", 9))
        self.saida_entry.grid(row=1, column=0, columnspan=2, sticky="ew")

        self.saida_button = ttk.Button(self.opcoes_frame, text="Salvar Como...", state="disabled",
        ...
        self.run_button = ttk.Button(action_frame, text="▶ Executar Limpeza", command=self.run_script,
        ...
        if modo == "arquivo":
            if self.in_place_var.get():
                if not messagebox.askyesno(
                        "Confirmação de Segurança (Arquivo)",
            ...
            if not messagebox.askyesno(
                    "Confirmação de Segurança (PASTA)",
                    f"Você está prestes a MODIFICAR RECURSIVAMENTE os arquivos na pasta:\n\n{entrada}\n\nEsta ação não pode ser desfeita e afetará TODOS os arquivos compatíveis (ex: .txt, .md) dentro dela.\n\nDeseja continuar?",
                    icon='warning'
            ):
                self.log("Operação cancelada pelo usuário.")
                return
