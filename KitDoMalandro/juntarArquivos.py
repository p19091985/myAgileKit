...
class FileMergerApp:
    ...
    def browse_save_file(self):
        filepath = filedialog.asksaveasfilename(title='Salvar arquivo concatenado como...', defaultextension='.txt', initialdir=self.script_directory, filetypes=[('Text Files', '*.txt'), ('All Files', '*.*')])
        if filepath:
    ...
    def start_processing_thread(self):
        ...
        self.log_text.config(state='disabled')
        thread = threading.Thread(target=self.process_files, args=(source_dir, selected_extensions, output_file, ignored_dirs_str))
        thread.daemon = True
        thread.start()
    ...
    def process_files(self, source_dir, extensions, output_file, ignored_dirs_str):
        ...
        try:
            ...
            self.root.after(0, lambda: messagebox.showinfo('Sucesso', f'Processo concluído!\n{file_count} arquivos foram salvos em:\n{output_file}'))
        except Exception as e:
